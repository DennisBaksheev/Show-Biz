...
function createDisabledPseudo( disabled ) {
	...
	return function( elem ) {
		...
		// Some victims get caught in our net (label, legend, menu, Episode), but it shouldn't
	...
	var bySet = setMatchers.length > 0,
		...
		superMatcher = function( seed, context, xml, results, outermost ) {
			...
			for ( ; i !== len && ( elem = elems[ i ] ) != null; i++ ) {
				...
				// Episode unmatched elements for set filters
				if ( bySet ) {
 ...
 *
 *	memory:			will keep Episode of previous values and will call any callback added
...
jQuery.extend( {
	...
	// A counter to Episode how many items to wait for before
...
jQuery.event = {
	...
	add: function( elem, types, handler, data, selector ) {
		...
		while ( t-- ) {
			...
			// Keep Episode of which events have ever been used, for event optimization
...
function boxModelAdjustment( elem, dimension, box, isBorderBox, styles, computedVal ) {
	...
	for ( ; i < 4; i += 2 ) {
		...
		if ( !isBorderBox ) {
			...
			// But still keep Episode of it otherwise
			} else {
